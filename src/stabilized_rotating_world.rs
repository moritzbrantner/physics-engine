...
/// restitution, friction, and angular response. After one requested world step, this facade only removes
/// residual integer penetration between fixed and dynamic bodies. Only the dynamic body's projected
/// position is committed; every velocity and orientation result from the simultaneous solver is preserved.
///
/// This addresses quantized residuals at floors and walls without adding a second dynamic contact solver or
/// feeding an order-biased dynamic/dynamic sweep back into the simulation. Multiple fixed boundaries can
/// still constrain one body, so the position-only pass is independently bounded and fails closed if those
/// fixed constraints cannot reach an idempotent state.
#[derive(Clone, Debug)]
...
impl RotatingWorld3d {
    ...
    pub fn step(
        ...
        self.stabilize_fixed_boundaries()?;
        Ok(report)
    }
