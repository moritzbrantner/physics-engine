...
use crate::{
    AngularState3d, AngularVelocity3d, BodyId, Orientation3d, OrientedBox3d,
    OrientedBoxError3d, RigidBody, RigidBox3d, RigidBoxFreeFlightConfig3d,
    RigidBoxFreeFlightError3d, RotatingContactSearchConfig3d, RotatingContactSearchError3d,
    Vec3i, obb_contact_seed,
};
...
fn stationary_grid_reuses_two_preparations_and_one_sat_evaluation() {
    let left = RigidBox3d::new(
        RigidBody::dynamic(BodyId(1), Vec3i::new(-4, 0, 0), Vec3i::ZERO, Vec3i::new(2, 2, 2)),
    ...
    for numerator in 1..=32 {
        let left = geometry.sample_geometry(&left, search, numerator, 32).unwrap();
        let right = geometry.sample_geometry(&right, search, numerator, 32).unwrap();
...
fn every_shape_component_and_pair_order_invalidates_the_exact_result() {
    ...
    let enlarged = OrientedBox3d { half_extents: Vec3i::new(3, 2, 2), ..left };
    let pairs = [
        (left, right),
...
fn cached_valid_contact_does_not_hide_invalid_geometry_or_error_order() {
    ...
    let invalid = OrientedBox3d { half_extents: Vec3i::new(0, 2, 2), ..valid };
    ...
    let mut contacts = PairContactCache3d::default();
    contacts.contact(&PreparedObb3d::new(valid), &PreparedObb3d::new(valid)).unwrap();
    for (left, right) in [
    ...
    assert_eq!(
        ...
        Err(RotatingContactSearchError3d::Contact(OrientedBoxError3d::InvalidHalfExtents)),
    );
}
