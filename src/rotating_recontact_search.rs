...
/// A single exact result for one pair traversal. No contact history survives a search or response.
#[derive(Debug, Default)]
...
pub fn sampled_rotating_recontact_search(
    ...
    Ok(sampled_rotating_recontact_search_with_broad_phase(boxes, config, &mut broad_phase)?.hit)
}
...
fn search_pair(
    ...
    let initially_contacting = contacts.contact(&prepared_left, &prepared_right)?.is_some();
    let denominator = u32::from(config.sample_count);
