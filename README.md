...
- deterministic rotating OBB normal response with linear/angular effective mass and restitution;
...
The current `World` simulation deliberately remains AABB-only and translational. Sphere support, OBB SAT, rotational sweep bounds, angular state, rotating-box free flight, rotational broad phase, sampled rotating contact search, shared sampled contact frontiers, and rotating normal response are independent foundations. They do not yet constitute a repeated-event rotational world solver. Endpoint-only sweep bounds retain their stated trajectory precondition; the rotating-box free-flight envelope widens them conservatively so acceleration and velocity reversals cannot hide sampled contacts.
...
Rotating normal response is also deliberately narrower than a full solver. It recomputes exact OBB contact geometry at the current state, measures contact-point velocity including angular motion, and applies one equal-and-opposite normal impulse. Ambiguous zero-depth contacts tied across multiple SAT axes remain observation-only rather than inventing torque from feature ordering. Friction, multi-contact iteration, position correction, and repeated remaining-time advancement remain separate stages.
...
ECS / game / simulation consumer
    ...
    physics-engine
          ...
          +-- rotating normal impulse response
          +-- spatial queries
...
1. repeated-event rotating-box stepping over the shared frontier and response primitive, preserving the explicit sampled-versus-analytic CCD boundary;
