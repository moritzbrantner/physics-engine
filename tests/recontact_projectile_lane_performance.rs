...
fn projectile_lane(body_count: u64) -> Vec<RigidBox3d> {
    ...
    // Deliberately give the earliest obstacle the largest id. Pair-major traversal has to inspect the
    // later obstacles first; globally sample-ordered search can stop the whole candidate set together.
    boxes.push(fixed(body_count, Vec3i::new(-20, 0, 0)));
    boxes
}

#[test]
fn thirty_six_body_projectile_lane_selects_the_front_loaded_contact() {
    let boxes = projectile_lane(36);
    assert_eq!(boxes.len(), 36);
    let hit = sampled_rotating_recontact_search(&boxes, config())
        .expect("valid 36-body re-contact search")
...
fn thirty_to_forty_body_projectile_lane_benchmark() {
    for body_count in [30_u64, 36, 40] {
        ...
        println!(
            "sample-major projectile lane: bodies={body_count}, median={median:?}, selected={}-{}",
