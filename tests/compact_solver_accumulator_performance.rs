...
impl PriorBTreeAccumulator {
    fn accumulate(&mut self, axis: Axis3, delta: Delta3) {
        ...
        for (target, value) in group.sum.iter_mut().zip(delta) {
            *target = target.checked_add(value).expect("benchmark delta fits i128");
        }
        group.count = group.count.checked_add(1).expect("benchmark count fits u32");
    }
...
fn compact_solver_accumulator_benchmark() {
    ...
    assert_eq!(run_compact(&mut compact, passes), run_prior(&mut prior, passes));
    ...
    for _ in 0..iterations {
        compact_checksum = compact_checksum.wrapping_add(run_compact(black_box(&mut compact), passes));
    }
