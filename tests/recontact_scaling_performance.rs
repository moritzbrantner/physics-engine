...
fn print_series(name: &str, measurements: &[Measurement]) {
    eprintln!(
        "recontact_scaling,pattern,bodies,candidate_pairs,median_ns,ns_per_pair,time_ratio,empirical_exponent"
    );
    for (index, measurement) in measurements.iter().enumerate() {
        ...
        let (time_ratio, exponent) = if let Some(previous) = index
            .checked_sub(1)
            .and_then(|previous| measurements.get(previous))
        {
        ...
        eprintln!(
            ...
            measurement.bodies, measurement.candidate_pairs, measurement.median_ns,
        );
    }
}
...
fn scaling_fixtures_expose_linear_and_quadratic_candidate_shapes() {
    ...
    for body_count in SIZES {
        ...
        assert!(
            chain_pairs.len()
                >= usize::try_from(body_count.saturating_sub(1))
                    .expect("benchmark body count fits usize"),
            "touching chain must retain at least the adjacent contacts"
        );
        assert!(
            chain_pairs.len()
                <= usize::try_from(body_count * 2).expect("benchmark body count fits usize"),
        ...
        let expected_dense_pairs = usize::try_from(body_count * (body_count - 1) / 2)
            .expect("benchmark pair count fits usize");
        assert_eq!(
            dense_pairs.len(),
