...
if grep -Eq '(^|/)(current_contact_query\.rs|rotating_world\.rs|stabilized_rotating_world\.rs|support_query\.rs)$' <<<"$changed"; then
...
if grep -Eq '(^|/)(rotating_recontact_search\.rs|rotating_recontact_search_ordered\.rs|rotating_recontact_cache_performance\.rs|recontact_scaling_performance\.rs|recontact_projectile_lane_performance\.rs)$' <<<"$changed"; then
