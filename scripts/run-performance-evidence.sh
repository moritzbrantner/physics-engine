...
if grep -Eq '(^|/)(rotating_world\.rs|rigid_box\.rs|solver_partition_performance\.rs)
if grep -Eq '(^|/)(ecs_world\.rs|rotating_world\.rs|stabilized_rotating_world\.rs|relaxed_rotating_world\.rs|precise_step_writeback\.rs)$' <<<"$changed"; then
  run_integration precise_step_writeback
fi
...
if grep -Eq '(^|/)(current_contact_query\.rs|rotating_world\.rs|stabilized_rotating_world\.rs|relaxed_rotating_world\.rs|support_query\.rs|precise_body_contact_query\.rs)
if grep -Eq '(^|/)(ecs_world\.rs|rotating_world\.rs|stabilized_rotating_world\.rs|relaxed_rotating_world\.rs|precise_step_writeback\.rs)$' <<<"$changed"; then
  run_integration precise_step_writeback
fi
