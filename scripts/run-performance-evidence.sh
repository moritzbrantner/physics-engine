...
if grep -Eq '(^|/)(rotating_world\.rs)if grep -Eq '(^|/)(current_contact_query\.rs|rotating_world\.rs|stabilized_rotating_world\.rs|relaxed_rotating_world\.rs|support_query\.rs|precise_body_contact_query\.rs)$' <<<"$changed"; then
